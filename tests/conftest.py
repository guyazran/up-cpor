...
_workspace = os.path.normpath(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))


def _is_workspace(p: str) -> bool:
    if not p:  # empty string means CWD
        return os.path.normpath(os.getcwd()) == _workspace
    return os.path.normpath(p) == _workspace


sys.path[:] = [p for p in sys.path if not _is_workspace(p)]


# ---------------------------------------------------------------------------
# DOT graph parsing and comparison helpers (shared by test_cpor and test_meta_cpor)
# ---------------------------------------------------------------------------
